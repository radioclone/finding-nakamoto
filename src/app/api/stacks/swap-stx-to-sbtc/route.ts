...
const constructSwapTx = async (
  ...
  const functionArgs: ClarityValue[] = [
    uintCV(stxAmount.toString()),
  ];

  const txOptions: any = {
    contractAddress,
    contractName,
    functionName: "swap-stx-to-sbtc",
    functionArgs,
    publicKey,
    fee,
    network,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Default to 1 STX (1,000,000 microSTX)
    const stxAmount =
      body.stxAmount !== undefined
        ? BigInt(typeof body.stxAmount === "string" ? body.stxAmount : Math.floor(body.stxAmount))
        : BigInt(1_000_000);
    ...
    console.error("Swap STX to sBTC failed:", error);
